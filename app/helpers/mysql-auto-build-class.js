...
class sql {
  ...
  where = (key, value, op = "=", rel = "and") => {
    ...
    if (that._whereList[0]["notUseArray"] === true)
      that._whereList[0]["notUseArray"] = false;
  ...
  whereSet = (whereStr) => {
    let that = this;
    //直接来个新的，原来的匹配被自动清除
    that._whereList = [{ notUseArray: true }];
  ...
  deleteAllSet = () => {
    ...
    that._whereList = [{ notUseArray: false }];
    that._whereStr = "";
    return that;
  };
  /**
  ...
  examine = () => {
    ...
    if (typeof that._table != "string")
      ...
      that._orderBy.rule != "ASC" &&
      that._orderBy.rule != "DESC" &&
      this._orderBy != ""
    )
      return {
        error: true,
      ...
      that._whereList[0].notUseArray == true &&
      typeof that._whereStr != "string"
    )
      return {
        error: true,
  ...
  update = (callback) => {
    ...
    sql +=
      ...
      " = " +
      that._data[arrKey[arrKeyLength - 1]] +
      " WHERE ";
    if (that._whereBinary === true) sql += "BINARY ";
    if (that._whereList[0]["notUseArray"] === true) {
      sql += that._whereStr + " ";
