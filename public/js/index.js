...
down.onclick = function () {
  ...
  down.timer = setInterval(function () {
    document.documentElement.scrollTop =
      document.documentElement.scrollTop +
      dir *
        Math.ceil(Math.abs(document.documentElement.scrollTop - headerH) / 10);
    if (
      Math.abs(document.documentElement.scrollTop - headerH) <= 10 ||
      document.documentElement.scrollTop + offsetH + 20 >= scrollH
    ) {
      clearInterval(down.timer);
    }
  }, 10);
...
function adjustFontSize() {
  if (clientW <= 1200 && clientW > 500) {
    ...
    authorName.style.fontSize = authorUrlGithub.style.fontSize = authorUrlBilibili.style.fontSize =
      "20px";
  } else if (clientW <= 500) {
...
var searchMethod = function () {
    window.location.href="/archives/search?search=" + search.value; 
  //   axios.get("/search?search=" + search.value).then(function (response) {
  //     console.log(response);
  //     console.log(search.value);
  //     console.log("test: searchBtn");
  //   });
  };
searchBtn.onclick = searchMethod;
